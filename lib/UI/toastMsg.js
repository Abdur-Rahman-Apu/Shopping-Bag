...
const showToastMsg = (msg, action, duration) => {
  ...
  // setStyleIntoTheUI(toastMsgLine, { width: "100%" });
  ...
  const toastHtml = `
          <div class="toast-msg-wrapper ${toastId}">
  ...
  setStyleIntoTheUI(selectElem(toastMsgContainer.querySelector(`${toastId}`)), {
    backgroundColor: isSuccess ? "#60e04c" : "rgb(239, 108, 108)",
  });

  console.log(toastMsgLine, "toastMsgline");
...
const hideToastBox = (toastId) => {
  Array.from(toastMsgContainer.children).forEach((elm) => {
    ...
    elm.classList.contains(`${toastId}`) && removeHtmlElm(elm);
  });
};
